...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      ...
      const transactionsData = resp.data.transactions;
      const balanceData = resp.data.balance;

     const newTransactions =  transactionsData.map( (transaction: Transaction) => {

        var valueformated;

        if(transaction.type === 'outcome'){
          valueformated = `- ${formatValue(transaction.value)}`;
        }else{
          valueformated = formatValue(transaction.value);
        }
